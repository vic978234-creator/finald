...
# --- API KEY ---
# KOBIS API에서 발급받은 두 가지 키를 여기에 입력하세요.
# -----------------------------------------------------------
# 1. 영화 목록 (LIST) API 키: searchMovieList 호출에 사용
KOBIS_LIST_KEY = "6350d8964d4c5160f40135185663cb48" # <--- 발급받은 첫 번째 키 입력

# 2. 영화 상세 정보 (DETAIL) API 키: searchMovieInfo 호출에 사용
KOBIS_DETAIL_KEY = "cc5c76f4946f878b829af9b116062ad4" # <--- 발급받은 두 번째 키 입력
# -----------------------------------------------------------
...
def fetch_movie_list(list_key):
    ...
    """
    if list_key == "YOUR_LIST_API_KEY_HERE":
        st.error("🚨 영화 목록 키를 'KOBIS_LIST_KEY' 변수에 입력해야 실제 데이터를 가져올 수 있습니다. 작업을 중단합니다.")
        return None
...
def get_full_analysis_data(list_key, detail_key):
    ...
    for i, movie in enumerate(movie_list_data):
        ...
        if detail_info:
            audi_cnt = int(detail_info.get('audiCnt', '0').replace(',', ''))
...
def analyze_hitmaker_index(movie_records, entity_type='Director'):
    ...
    for movie in movie_records:
        ...
        for entity_tuple in entities:
            # 감독: (이름, 영화제목, 관객수), 회사: (이름, 영화제목, 관객수, 역할)
            entity_name = entity_tuple[0]
            # movie_title = entity_tuple[1] # 현재 사용하지 않음
            audience = entity_tuple[2]
            
            # 관객수가 0인 영화는 흥행 분석에서 제외 (영향력이 없다고 간주)
            if audience > 0:
    ...
    for name, data in entity_data.items():
        # 최소 1개 이상의 영화에 참여했고, 관객수가 0보다 큰 경우만 포함
...
def main():
    ...
    with col1:
        ...
        # 세션 상태에 초기 실행 여부를 저장하여 한 번은 실행되도록 합니다.
    ...
    if analyze_button or st.session_state['initial_run']:
        if st.session_state['initial_run']:
            st.session_state['initial_run'] = False # 초기 실행 후 상태 변경
